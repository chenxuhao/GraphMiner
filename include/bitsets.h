...
public:
	...
	void alloc(int n, int nbits) {
		...
		for (int i = 0; i < n; i++) {
			CUDA_SAFE_CALL(cudaMalloc(&h_bit_vectors[i], vec_size() * sizeof(uint64_t)));
			reset(i);
		}
		CUDA_SAFE_CALL(cudaMalloc(&d_bit_vectors, n * sizeof(uint64_t*)));
		CUDA_SAFE_CALL(cudaMemcpy(d_bit_vectors, h_bit_vectors, n * sizeof(uint64_t*), cudaMemcpyHostToDevice));
	}
	void clear() {
		for (int i = 0; i < num_sets; i++) reset(i);
		CUDA_SAFE_CALL(cudaMemcpy(d_bit_vectors, h_bit_vectors, num_sets * sizeof(uint64_t*), cudaMemcpyHostToDevice));
	}
	void clean() {
	...
	void reset(int i) {
		CUDA_SAFE_CALL(cudaMemset(h_bit_vectors[i], 0, vec_size() * sizeof(uint64_t)));
	}
	__device__ void set(int sid, int bid) {
		...
		if ((d_bit_vectors[sid][bit_index] & bit_offset) == 0) { // test and set
			atomicOr((unsigned long long int*)&d_bit_vectors[sid][bit_index], bit_offset);
		}
	}
