...
__forceinline__ __device__ unsigned list_smaller(vidType bound, vidType *in, vidType size_in, vidType *out) {
...
template <typename T = vidType>
__forceinline__ __device__ T intersect_num_merge(T* a, T size_a, T* b, T size_b, T* cache) {
  int thread_lane = threadIdx.x & (WARP_SIZE-1);
  int warp_lane   = threadIdx.x / WARP_SIZE;     // warp index within the cta
  int p = warp_lane * WARP_SIZE;
  T count = 0;
...
__forceinline__ __device__ T intersect_num_merge(T* a, T size_a, T* b, T size_b) {
  __shared__ T cache[BLOCK_SIZE];
  return intersect_num_merge(a, size_a, b, size_b, cache);
}

template <typename T = vidType>
__forceinline__ __device__ T intersect_num_merge(T* a, T size_a, T* b, T size_b, T upper_bound) {
  int thread_lane = threadIdx.x & (WARP_SIZE-1);
  vidType count = 0;
  auto a_off = thread_lane; 
  auto b_off = thread_lane; 
