...
__global__ void __launch_bounds__(BLK_SZ, 4)
clique8_warp_edge(eidType ne, GraphGPU g, vidType *vlists, vidType max_deg, AccType *total) {
    ...
    for (vidType idx1 = 0; idx1 < list_size[warp_lane][0]; idx1++) {
      ...
      for (vidType idx2 = 0; idx2 < list_size[warp_lane][1]; idx2++) {
        ...
        for (vidType idx3 = 0; idx3 < list_size[warp_lane][2]; idx3++) {
          ...
          auto count4 = intersect(vlist+max_deg*2, w3_size, g.N(v4), v4_size, vlist+max_deg*3);
          if (thread_lane == 0) list_size[warp_lane][3] = count4;
          __syncwarp();
          for (vidType idx4 = 0; idx4 < list_size[warp_lane][3]; idx4++) {
            vidType v5 = vlist[max_deg*3+idx4];
            vidType v5_size = g.getOutDegree(v5);
            vidType w4_size = list_size[warp_lane][3];
            auto count5 = intersect(vlist+max_deg*3, w4_size, g.N(v5), v5_size, vlist+max_deg*4);
            if (thread_lane == 0) list_size[warp_lane][4] = count5;
            __syncwarp();
            for (vidType idx5 = 0; idx5 < list_size[warp_lane][4]; idx5++) {
              vidType v6 = vlist[max_deg*4+idx5];
              vidType v6_size = g.getOutDegree(v6);
              vidType w5_size = list_size[warp_lane][4];
              counter += intersect_num(vlist+max_deg*4, w5_size, g.N(v6), v6_size);
            }
          }
        }
      }
