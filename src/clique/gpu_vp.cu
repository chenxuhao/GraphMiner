...
#include "clique4_warp_vertex.cuh"
#include "clique5_warp_vertex.cuh"
...
void CliqueSolver(Graph &g, int k, uint64_t &total, int, int) {
  ...
  int max_blocks_per_SM = maximum_residency(clique4_warp_vertex, nthreads, 0);
  if (k==5) max_blocks_per_SM = maximum_residency(clique5_warp_vertex, nthreads, 0);
  ...
  t.Start();
  if (k == 4) {
    clique4_warp_vertex<<<nblocks, nthreads>>>(nv, gg, frontier_list, md, d_total);
  } else if (k == 5) {
    clique5_warp_vertex<<<nblocks, nthreads>>>(nv, gg, frontier_list, md, d_total);
  } else if (k == 6) {
