...
__global__ void __launch_bounds__(BLK_SZ, 5)
clique7_warp_edge(eidType ne, GraphGPU g, vidType *vlists, vidType max_deg, AccType *total) {
      ...
      for (vidType idx2 = 0; idx2 < list_size[warp_lane][1]; idx2++) {
        ...
        for (vidType idx3 = 0; idx3 < list_size[warp_lane][2]; idx3++) {
          ...
          for (vidType idx4 = 0; idx4 < list_size[warp_lane][3]; idx4++) {
            ...
            counter += intersect_num(vlist+max_deg*3, w4_size, g.N(v5), v5_size);
          }
        }
      }
    }
