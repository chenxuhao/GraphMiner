...
void automine_3star2staredge(Graph &g, std::vector<std::vector<uint64_t>> &global_counters) {
  #pragma omp parallel
  {
    ...
    for(vidType v0 = 0; v0 < g.V(); v0++) {
      VertexSet y0 = g.N(v0);
      for (auto v1 : y0) {
        VertexSet y0y1 = g.N(v1);
        counter[0] += nChoosek(y0.size()-1,3)*nChoosek(y0y1.size(),2);
      }
    }
}
}
...
void MotifSolver(Graph &g, int k, std::vector<uint64_t> &total, int, int) {
  ...
  Timer t;
  t.Start();
  automine_3star2staredge(g, global_counters);
  for (int tid = 0; tid < num_threads; tid++)
