...
void automine_tt(Graph &g, std::vector<std::vector<uint64_t>> &global_counters) {
  #pragma omp parallel
  {
    ...
    for (vidType u = 0; u < g.V(); u ++) {
      auto adj_u = g.N(u);
      for (auto v : adj_u) {
        counter[0] += (uint64_t)intersection_num(adj_u, g.N(v))*(g.N(u).size()-2);
      }
    }
  }
}
