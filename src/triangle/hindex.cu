...
void TCSolver(Graph &g, uint64_t &total, int, int) {
  ...
  int max_blocks_per_SM = maximum_residency(hindex_warp_edge, nthreads, 0);
  ...
  Timer t;
  t.Start();
  hindex_warp_edge<<<nblocks, nthreads>>>(ne, gg, bins, d_total);
  CUDA_SAFE_CALL(cudaDeviceSynchronize());
  t.Stop();

  std::cout << "runtime [gpu_hindex] = " << t.Seconds() << " sec\n";
